...
uses  CProgramas, UUpdatedApConv, CConfere, CAgsINI, CAgsSAT;
...
begin
  ...
  with QrClientes do
    begin
      ...
      DatabaseName:= 'ags';
      SQl.Clear;
      SQL.Text:= 'ALTER TABLE CLI000 DROP COLUMN ' + '''' + 'comissão' + '''';
      ExecSQL;
  ...
  try
    ...
    begin
      ...
      if (Pos('ERRADO',F.Name) > 0)  or (Pos('_.DBF',F.Name) > 0) or (Pos('.TXT',F.Name) > 0)    or (Pos('.EXE',F.Name) > 0)     or
         (Pos('.NDX',F.Name) > 0)    or (Pos('.NTX',F.Name) > 0)  or (Pos('CÓPIA',F.Name) > 0)   or (Pos('-',F.Name) > 0)        or
