...
Begin
   ...
   Session.GetAliasParams('AGS',WAlias);   // Fluxo_de_Caixa é o Alias a ser retornado
   ...
   wAlias := TStringList.Create;
   Session.GetAliasParams('AGS_',WAlias);   // Fluxo_de_Caixa é o Alias a ser retornado
   WServidor := WAlias[0];                        // o caminho
   WServidor := Copy(WServidor,6,255);
   WServidor := Copy(WServidor,1,(Length(WServidor)));
   file_1.Directory := WServidor;
   ...
   ShowMessage('CONFERÊNCIA FINALIZADA');
end;
